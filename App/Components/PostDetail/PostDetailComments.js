...
import { heartFillsvg, heartSvg } from '../../../assets/svgs/svgs';

export default function PostDetailComments({comment, index}) {
  const [userData, setUserData] = useState(null);
  const [isLiked, setIsLiked] = useState(false);

  const heartIconFill = () => (
    <SvgXml xml={heartFillsvg} width="16" height="16" color={'black'} />
  );
  const heartIcon = () => (
    <SvgXml xml={heartSvg} width="16" height="16" color={'black'} />
  );
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch(
          // `http://192.168.0.116:3000/api/usersR/user/${comment.userId}`,
          `http://192.168.1.103:3000/api/usersR/user/${comment.userId}`,
        );
  ...
  if (!userData) {
    return <Text>No comments for this Post 🙂</Text>;
  } else {
    const {username, image} = userData;
    return (
      <View style={tailwind`flex flex-row items-start mt-4`}>
        <Image
          style={tailwind`w-12 h-12 rounded-full mr-3`}
          source={{uri: image}}
        />
        <View style={tailwind`flex-1`}>
          <Text style={tailwind`mb-1 font-semibold text-gray-700`}>
            {username}
          </Text>
          <Text style={tailwind`text-sm text-gray-600`}>{comment.text}</Text>
        </View>
